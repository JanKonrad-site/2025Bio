function generateCode() {
  ...
  function getExpressionFromPrint(editor) {
    const parts = [];
    editor.childNodes.forEach(node => {
      if (node.nodeType === Node.TEXT_NODE) {
        const txt = node.textContent.trim();
        if (txt) parts.push(`"${sanitize(txt)}"`);
        ...
        parts.push(node.textContent.trim());
      } else if (node.classList.contains('segment-operator-wrapper')) {
  ...
  function parseCanvas(container, indent = '') {
    ...
    blocks.forEach(block => {
      if (block.classList.contains('type-var')) {
        ...
        const expr = getExpressionFromPrint(editor);
        codeLines.push(`${indent}print(${expr || ''})`);
        ...
        const elifBlocks = block.querySelectorAll('.else-if-block');
        ...
        elifBlocks.forEach(elif => {
          const cond = getExpressionFromCondition(elif.querySelector('.condition-editor'));
          codeLines.push(`${indent}elif ${cond}:`);
          parseCanvas(elif.querySelector('.elif-canvas'), indent + '    ');
        });
